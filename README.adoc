...
image::https://img.shields.io/maven-central/v/net.openhft/chronicle-salt.svg[Chronicle Salt on Maven Central]
...
* JDK 8 or https://www.oracle.com/java/technologies/downloads/[higher]
...
OS X users can get libsodium via https://brew.sh/[Homebrew] with:
...
=== Private (Secret) Keys
 ...
 - In Ed25519, the secret key is used to sign messages, not to decrypt them.
 - Keep the secret key private; do not share it with verifiers.
 - https://www.techopedia.com/definition/16135/private-key[Detailed definition]
 
=== Public Keys

  - One of two keys used in public key cryptography.
  - In Ed25519, the public key is used to verify message signatures and can be shared.
...
Verification checks the message and signature against the signer's public key; it does not require the secret key.
...
Ed25519 uses the secret key to sign a message. Verification checks whether the signature is valid for that message
and the corresponding public key. It does not decrypt an encrypted hash.

Signing does not encrypt the message or provide confidentiality. In the example above, `signatureAndMsg` contains
both the signature and the unencrypted message.

To authenticate a particular sender, the recipient must also have a trustworthy association between that sender
and the public key. Successful verification alone does not establish who owns the key.

See https://doc.libsodium.org/public-key_cryptography/public-key_signatures[Libsodium's public-key signature documentation]
for the distinction between signatures and authenticated encryption.
...
Chronicle Bytes :: A similar purpose to Java NIO's ByteBuffer, but with added extensions.        https://github.com/OpenHFT/Chronicle-Bytes[View Chronicle-Bytes here]
