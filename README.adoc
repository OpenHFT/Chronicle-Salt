...
Once `sign` has been called the `MultiPart` object should not be used further.
...
`Verify` will throw an `IllegalStateException` is the call fails for any reason.
Once `verify` has been called the `MultiPart` object should be used further.
