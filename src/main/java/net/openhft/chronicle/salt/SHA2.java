...
import static net.openhft.chronicle.salt.Sodium.SIZEOF_CRYPTO_HASH_SHA256_STATE;
import static net.openhft.chronicle.salt.Sodium.SIZEOF_CRYPTO_HASH_SHA512_STATE;
...
public enum SHA2 {
    ...
    public static class MultiPartSHA256
    {
        ...
        {
            checkValid(Sodium.SODIUM.crypto_hash_sha256_update( state.addressForRead(0),
        ...
        {
            result = Sodium.Util.setSize( result, 	HASH_SHA256_BYTES );
            checkValid(Sodium.SODIUM.crypto_hash_sha256_final( state.addressForRead(0),
                    result.addressForWrite(0) ),
                    "Multi-part SHA256 failed" );
    ...
    public static class MultiPartSHA512
    {
        ...
        public void add( BytesStore message )
        {
            checkValid(Sodium.SODIUM.crypto_hash_sha512_update( state.addressForRead(0),
                    message.addressForRead(message.readPosition()),
                    message.readRemaining() ),
                    "Failed to add to multi-part message");
        }
        ...
        {
            result = Sodium.Util.setSize( result, 	HASH_SHA512_BYTES );
            checkValid(Sodium.SODIUM.crypto_hash_sha512_final( state.addressForRead(0),
                    result.addressForWrite(0) ),
                    "Multi-part SHA512 failed" );
