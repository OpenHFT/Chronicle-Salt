...
public enum Ed25519 {
	 ...
	 * long privateKeyAddr = privateKey.addressForRead(0); long secretKeyAddr =
	 ...
	 * Sodium.SODIUM.crypto_box_curve25519xsalsa20poly1305_keypair(
	 * publicKeyAddr, privateKeyAddr), "secret key");
	...
	public static boolean verify(BytesStore<?, ?> sigAndMsg, BytesStore<?, ?> publicKey) {
		if (sigAndMsg.readRemaining() < 64) {
			throw new IllegalArgumentException("sigAAndMsg");
		}
		if (publicKey.readRemaining() != 32) {
