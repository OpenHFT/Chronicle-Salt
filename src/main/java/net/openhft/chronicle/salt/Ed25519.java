...
public enum Ed25519 {
    ...
    public static void privateToPublic(Bytes<?> publicKey, Bytes<?> privateKey) {
        ...
        Sodium.SODIUM.crypto_scalarmult_curve25519(publicKey.addressForWrite(0),
                privateKey.addressForRead(privateKey.readPosition()), Sodium.SGE_BYTES.addressForRead(0));
    ...
    public static void privateToPublicAndSecret(Bytes<?> publicKey, Bytes<?> secretKey, BytesStore seed) {
        ...
        if (seed.readRemaining() != SEED_LENGTH) {
            throw new IllegalArgumentException(
                    String.format(ERR_MEESAGE_PATTERN, "seed size", seed.readRemaining(), SEED_LENGTH));
        }
        SODIUM.crypto_sign_ed25519_seed_keypair(publicKey.addressForWrite(0), secretKey.addressForWrite(0),
                seed.addressForRead(seed.readPosition()));
        publicKey.readPositionRemaining(0, PUBLIC_KEY_LENGTH);
    ...
    public static void generateKey(Bytes<?> privateKey, Bytes<?> publicKey) {
        ...
        checkValid(Sodium.SODIUM.crypto_box_curve25519xsalsa20poly1305_keypair(publicKeyAddr, privateKeyAddr),
                "secret key");
    ...
    static class LocalEd25519 {
        ...
        void sign(Bytes sigAndMsg, BytesStore message, BytesStore secretKey) {
            ...
            checkValid(
                    SODIUM.crypto_sign_ed25519(sigAndMsg.addressForWrite(0), sigLen,
                            message.addressForRead(message.readPosition()), msgLen, secretKey.addressForRead(0)),
                    "Unable to sign");
        ...
        boolean verify(BytesStore sigAndMsg, BytesStore publicKey) {
            ...
            int ret = SODIUM.crypto_sign_ed25519_open(buffer.addressForWrite(0), sigLen,
                    sigAndMsg.addressForRead(sigAndMsg.readPosition()), (int) sigAndMsg.readRemaining(),
